from main import SessionLocal, Book, Student, Loan, engine, Base
...
for b in books_data:
    existing_copies = db.query(Book).filter(Book.isbn == b["isbn"]).count()
    new_book = Book(**b, copy_number=existing_copies + 1)
    db.add(new_book)
...
# 3. 產生一些借閱紀錄 (包含正常與逾期)
s1 = db.query(Student).filter(Student.student_id == "S001").first()
s2 = db.query(Student).filter(Student.student_id == "S002").first()
b1 = db.query(Book).filter(Book.id == 1).first()
b2 = db.query(Book).filter(Book.id == 3).first()
